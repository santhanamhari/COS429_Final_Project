...
class Solu(object):
    ...
    def gen_output(self, out_dir, tag=''):
        if len(self.blinks) == 0 or len(self.blinks) > 5:
            print("FAKE")
        else:
            print("REAL")
    ...
    def push_eye_prob(self, eye1_prob, eye2_prob):
        #if eye1_prob >= 0.5 or eye2_prob >= 0.5:
            #print("GREATER")
        if len(self.blinks) == 0 or self.frame_num - self.blinks[len(self.blinks)-1] > 3:
            if eye1_prob >= 0.5 or eye2_prob >= 0.5:
                self.blinks.append(self.frame_num)
                print("GREATER")
        else:
            "nothing"
