...
def compute_accuracy(classifier, dirname, frame_subsample_count = 30):
    ...
    filenames = {}
    i = 0
    for folder in listdir(dirname):
        cwd = join(dirname, folder)
        for vid in listdir(cwd):
            if vid[-4:] == '.mp4' or vid[-4:] == '.avi' or vid[-4:] == '.mov':
                filenames[i] = listdir(cwd)
                i = i + 1
    
    i = 0 
    for i in range(len(filenames)):
        for vid in filenames[i]:
            print('Dealing with video ', vid)

            # Compute face locations and store them in the face finder
            print(join(filenames[0],vid))
            face_finder = FaceFinder(join(filenames[0], vid), load_first_face = False)
